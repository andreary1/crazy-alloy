...
import net.mcreator.countries.procedures.JellySnakePlayerCollidesWithThisEntityProcedure;
...
public class JellySnakeEntity extends Monster implements GeoEntity {
	...
	public JellySnakeEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(ClModEntities.JELLY_SNAKE.get(), world);
	}

	public JellySnakeEntity(EntityType<JellySnakeEntity> type, Level world) {
		super(type, world);
		xpReward = 2;
		setNoAi(false);
		setMaxUpStep(0.6f);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "jellysnake");
	}
	...
	protected void dropCustomDeathLoot(DamageSource source, int looting, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(ClModItems.DEAD_JELLY_SNAKE.get()));
	}
	...
	public static void init() {
		SpawnPlacements.register(ClModEntities.JELLY_SNAKE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("animation.jellysnake.walk"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("animation.jellysnake.idle"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(JellySnakeEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
